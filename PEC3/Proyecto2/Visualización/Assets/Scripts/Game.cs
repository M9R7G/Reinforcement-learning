...
public class Game: MonoBehaviour {
	...
	private GameObject[,] board_arrow;
	private Agent agent;
		...
		for (int i = 0; i < ROWS; i++)
			for (int j = 0; j < COLS; j++)
				if (board_int[i, j] == 0)
				{
					...
					{
						case 0:
							board_arrow[i, j] = Instantiate(prefabArrow, pos, Quaternion.Euler(90,90,0));  // Right  
							break;

						case 1:
							board_arrow[i, j] = Instantiate(prefabArrow, pos, Quaternion.Euler(90, 0, 0));   // Up
							break;

						case 2:
							board_arrow[i, j] = Instantiate(prefabArrow, pos, Quaternion.Euler(90, -90, 0));  // Left
							break;

						case 3:
							board_arrow[i, j] = Instantiate(prefabArrow, pos, Quaternion.Euler(90, 180, 0));  // Down
							break;
					}
				}
	}

	void UpdatBoardArrow()
    {
		// 0: rigth; 1: up; 2: left; 3: 
		...
		for (int i = 0; i < ROWS; i++)
			for (int j = 0; j < COLS; j++)
				if (board_int[i, j] == 0)
				{
					...
					{
						case 0:
							board_arrow[i, j].transform.rotation = Quaternion.Euler(90, 90, 0);  // Right  
							break;

						case 1:
							board_arrow[i, j].transform.rotation = Quaternion.Euler(90, 0, 0);   // Up
							break;

						case 2:
							board_arrow[i, j].transform.rotation = Quaternion.Euler(90, -90, 0);  // Left
							break;

						case 3:
							board_arrow[i, j].transform.rotation = Quaternion.Euler(90, 180, 0);  // Down
							break;
					}
				}
	}
	...
	void StartAgent() {
		...
		agent.GameSetup(ROWS, COLS); //Recode
	}
	...
	void NewTurn() {
		...
		// 4) Update arrows
		UpdatBoardArrow();
		// 5) If the episode ends then start a new one
		if (episodeEnds)
			StartEpisode ();

		// 6) Update learning texts on screen
	...
	*/
	...
	// 3) Combine both numbers to get all possible combinations (breakables*number of
	// possible agent positions + agent position)
