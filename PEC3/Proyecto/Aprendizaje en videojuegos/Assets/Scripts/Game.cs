...
public class Game: MonoBehaviour {
	...
	public float deathReward; // Actividad 4
	...
	private int enemyRow, enemyCol;		// To locate the enemy Actividad 4
	...
	private bool playerReached;			// True if the enemy reaches the agent
	...
	void Start () {
		// Start file Actividad 1
		using (System.IO.StreamWriter file =
			new System.IO.StreamWriter("data.csv", true))
		{
			file.WriteLine($"Episodio;Recompensa");
		}
	...
	void NewTurn() {
		...
		bool episodeEnds = goalReached || turn >= episodeTurns || playerReached; // Actividad 2
	...
	private int ComputeState() {
		...
		int state = stateBreakables*ROWS*COLS*ROWS*COLS + agentPosition*ROWS*COLS+ enemyPosition;
		return state;
	}

	// Move enemy according to the action performed (if possible)
