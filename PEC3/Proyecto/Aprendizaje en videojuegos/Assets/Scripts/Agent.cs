...
public class Agent : MonoBehaviour {
	...
	public void GameSetup(int nRows, int nCols, int nBreakables) {
		...
		// c) Position of the enemy on the board (rows x cols)
		// The number of states is the product of a), b) and c) (all possible combinations)

		rows       = nRows;
		cols       = nCols;
		breakables = nBreakables;

		states     = rows * cols * rows * cols * Mathf.FloorToInt(Mathf.Pow(2, breakables));
